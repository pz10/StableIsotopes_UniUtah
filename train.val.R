...
myfile <- paste0(path, "/bird.data.validation.csv")
...
# 1 kriging
ebk.mean <- raster(paste0(path, "/trainkrig"))
crs(ebk.mean) <- projection(wrld_simpl)

ebk.se <- raster(paste0(path, "/trainingse"))
...
write.csv(data,"birds_validation.csv")
#################################################################################
path <- "Z:/Group_workspace/Week_2_2016/Birds"
birds <- paste0(path, "/hobson_birds_2H_feath_precip.csv")
birds <- paste0("birds_validation.csv")
birds <- paste0("birds.csv")
# data <- fread("Hobson_bird_feathers.csv")
birds <- fread(birds)
mybirds <- birds[FID %in% c(454,778),]
mybirds <- birds
# mybirds <- data
# mydensity <- data.table(FID=as.numeric(NA), dens.vect = list(NA))
# mydensity <- data.table(FID=as.numeric(NA), dens.vect = as.list(x=as.numeric(NA), y=as.numeric(NA)))
# dens.p <- list(x=list(as.numeric(NA)), y=list(as.numeric(NA)))
# dens.p <- list(x=list(as.numeric(NA)), y=list(as.numeric(NA)))
# dens.p <- data.table(FID = as.numeric(NA),
#                         dens.x = list(as.numeric(NA)),
#                         dens.y = list(as.numeric(NA))
#                     
# )
dens <- data.table(FID=as.numeric(rep(NA, times= 512*nrow(mybirds))),
                   prob.precip=as.numeric(NA), dens.precip =as.numeric(NA),
                   prob.krieg=as.numeric(NA), dens.krieg =as.numeric(NA))
# precip assignment
count <- 1
for (i in 1:nrow(mybirds)){
        pp <- calcCellProb(mybirds$Delta2H[i],precip.rescale, sample(c(stdv, stdv),size=1, replace=T)) # compute probabilities
        top <- pp >= quantile(getValues(pp),na.rm=T,0.9)[[1]]  # find top 10% of pp
        np <- calcNormProb(pp)
        
        mybirds$nprob.precip[i] <- extract(np,cbind(mybirds$Longitude[i],mybirds$Latitude[i]))
        # bird.assign.precip <- raster::stack(bird.assign.precip, np)
        # top10.precip <- raster::stack(top10.precip, top)
        
        d <- density(np)
        
        dens[count:(count+511), FID:=as.numeric(mybirds$FID[i])]
        dens[count:(count+511), prob.precip:=d$x]
        dens[count:(count+511), dens.precip:=d$y]
...
count <- 1
for (i in 1:nrow(mybirds)){
        pp <- calcCellProb(mybirds$Delta2H[i],ebk.mean, stdv = ebk.se) # compute probabilities
        top <- pp >= quantile(getValues(pp),na.rm=T,0.9)[[1]]  # find top 10% of pp
        np <- calcNormProb(pp)
        
        mybirds$nprob.krieg[i] <- extract(np,cbind(mybirds$Longitude[i],mybirds$Latitude[i]))
        # bird.assign.krieg <- raster::stack(bird5assign.krieg, np)
        # top10.krieg <- raster::stack(top10.krieg, top)
        ...
        dens[count:(count+511), FID:=as.numeric(mybirds$FID[i])]
        dens[count:(count+511), prob.krieg:=d$x]
        dens[count:(count+511), dens.krieg:=d$y]
        
        count <- count+512
        
        cat(i," ")
}
dens[prob.precip<0, prob.precip:= NA]
dens[prob.precip>1, prob.precip:= NA]
dens[prob.krieg<0, prob.krieg:= NA]
dens[prob.krieg>1, prob.krieg:= NA]

write.csv(dens,"birds_densities.csv")
write.csv(dens,"birds_densities_2.csv")
...
library(boot)
set.seed(1234)
results <- boot(data=data$nprob.diff.krig_precp, statistic=mean, 
                R=1000)
...
plot(density(data$nprob.krieg, bw=0.03))
hist(data$nprob.krieg, na.rm=T, breaks=50, freq =F)

plot(density(data$nprob.precip, na.rm=T))
hist(data$nprob.precip, na.rm=T, breaks=50, freq =F)
